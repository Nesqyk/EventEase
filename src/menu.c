...
int auth_menu() 
{
    ...
    char *last_user_username = read_user(atoi(last_user_id), "full_name");
        ...
        else if (choice == 7) // Submit
        {
            ...
            if (register_user(new_user)) 
            {
                ...
                while (1) 
                {
                    ...
                    add_notification(new_user.id, "Welcome to Event Ease! Do enjoy your stay", "Unread");
                    char str_id[50];
...
int client_menu(int client_id)
{
    ...
    while(1)
    {
        ...
        if (count_unread == 0) {
            ...
            sprintf(notification_message, "\n         🔔 You have %s%d%s unread notification%s.         \n", 
                    GREEN, count_unread, RESET, count_unread > 1 ? "s" : ""); 
        }
        ...
        if(scanf("%d", &client_choice) != 1 || client_choice < 1 || client_choice > 7)
        {
                    ...
                    } else if(book_event_choice == 6) // confirm
                    {
                        if(book_event_filled >= 5 && book_input[0] != NULL && book_input[1] != NULL && book_input[2] != NULL && book_input[3] != NULL && book_input[4] != NULL)
                        {
                            while(1)
                            {
                                ...
                                char event_type[50], venue_choice[50], date_time[50], selected_package[50], display_cost[50], display_percentage[50], amount_due[50], duration [50];
                                ...
                                sprintf(date_time,"🕒 Date & Time        : %s", bk_event.event_date);
                                sprintf(duration, "⏳Event Duration      : %s - %s",bk_event.start_time, bk_event.end_time);
                                ...
                                if(scanf("%s", agree_balance) != 1 || strcasecmp(agree_balance, "y") == 0)
                                {
                                    ...
                                    sprintf(reminder_msg_organizer, "Review Event %d (1-3 days). Client: %s", bk_event.id, read_user(client_id, "full_name"));
                        ...
                        if (preview_pkg == NULL) 
                        {
                            ...
                            while (1) 
                            {
                                while (1) {
                                    ...
                                    printf("%s\n", preview_pkg);
                                    printf("=========================================\n");
                                    printf("Enter Package ID to Select (press 'E' to go back): ");
...
int organizer_menu(int organizer_id)
{
    ...
    while(1)
    {
        ...
        char *dashboard_choices[] = {
            "Manage Booked Events      ✅ - View, modify, or cancel reservations.",
            "Event Type Management     🎉 - Create, update, or cancel events.",
        ...
        if(scanf("%d", &organizer_choice) != 1 || organizer_choice < 1 || organizer_choice > 9)
        {
            printf("\nPlease enter a valid choice\n");
            Sleep(2000);
            clear_input_buffer();
        ...
        if(organizer_choice == 1)
        {
            ...
            while(1)
            {
                ...
                while(1)
                {
                    ...
                    } else
                    {
                        ...
                        while(1)
                        {
                            char *manage_booked_events_menu[] = {
                                "==================================================",
                                "             ✅ Manage Booked Events             ",
                                "==================================================",
                                "",
                                "📍 Home > Dashboard > Manage Booked Events",
                                "",
                                "==================================================",
                                NULL
                            };

                            char *manage_booked_events_options[] = {
                                "Update Status     🔄 - Change the current status of a booking ",
                                "Update Balance    💰 - Adjust the balance or payment details for a booking.",
                                "View Details      🔍 - See complete information about a specific booking.",
                                "Cancel Booking    ❌ - Cancel an existing booking.",
                                "Back to Dashboard 🔙 - Return to the main dashboard.",
                                NULL
                            };

                            display_options(NULL, manage_booked_events_menu, manage_booked_events_options, NULL);
                            if(scanf("%d", &menu_choice) != 1 || menu_choice < 1 || menu_choice > 5)
                            {
                                clear_input_buffer();
                                printf("\nPlease enter a valid choice \n");
                                Sleep(2000);
                            ...
                            if(menu_choice == 1) // update status
                            {
                                while(1)
                                {
                                    int status_choice; 
                                    const char *update_status_paragraph[] = {
                                        "==================================================",
                                        "              🛠️ Update Event Status              ",
                                        "==================================================",
                                        "📍 Home > Dashboard > Event Management > Update Event Status",
                                        "",
                                        "🔄 Updating the status of your event helps ensure",
                                    ...
                                    const char *update_status_options[] = {
                                        "Pending ⏳        - Mark the event as awaiting confirmation.",
                                        "Confirmed ✅      - Confirm the event.",
                                        "In Progress ⚙️    - Mark the event as under preparation.",
                                        "Incomplete Payment 💳❌ - Indicate payment issues.",
                                        "Ongoing 🚀        - Mark the event as currently happening.",
                                    ...
                                    int status = status_choice - 1;

                                    switch(status) 
                                    {
                                        case 0: sprintf(status_msg, "Pending ⏳"); break;
                                        case 1: sprintf(status_msg, "Confirmed ✅"); break;
                                        case 2: sprintf(status_msg, "In Progress ⚙️"); break;
                                        case 3: sprintf(status_msg, "Incomplete Payment 💳❌"); break;
                                        case 4: sprintf(status_msg, "Ongoing 🚀"); break;
                                        case 5: sprintf(status_msg, "Done 🎉"); break;
                                    }

                                    char notif_msg[50];

                                    sprintf(notif_msg, "Event ID: %d Updated to: %s", booked_event_id, status_msg);

                                    char status_str[5];
                                    sprintf(status_str, "%d", status);
                                    add_notification(user_id, notif_msg,"Unread");

                                    update_event(user_id, booked_event_id, "status", status_str);

                                    printf("\n==================================================\n");
                                    printf("              ✅ Event Status Updated             \n");
                                    printf("==================================================\n");
                                    printf("🎉 The event status has been successfully updated to %s.\n", status_msg);
                                    printf("==================================================\n");
                                    printf("\nPress [ANY KEY] to return to the previous menu.");
                                    getch(); // Pause to allow the user to read the message
                                    break;
                                }
                            } else if(menu_choice == 2) // update balance
                            {
                                int balance;
                                printf("\nEnter new balance (-1 to cancel): ");
                                if(scanf("%d", &balance) != 1 || balance < 0 && balance != -1)
                                {
                                    clear_input_buffer();
                                ...
                                update_event(user_id, booked_event_id, "balance", balance_str);
                                // preview user
                                // 
                            } else if(menu_choice == 3) // view details
                            {
                                while(1)
                                {
                                    system("cls");
                                    ...
                                    printf("Please enter any [KEY] to stop viewing\n");
                                    getch();
                                    break;
                                }
            ...
            // preview users first
            // then preview user's booked event
                // Update Status
                // Modify Details
        }
        else if(organizer_choice == 4) // dashboard overview
        {
            while(1)
            {
                ...
                } else if(organizer_choice == 3) // del;ete event
                {
                    ...
                    while(1)
                    {
                        ...
                        while(1)
                        {
                            ...
                            {
                                // put here :D
                                delete_typeevent(organizer_choice);
                                break;
                            } else if(confirmation == 'N' || confirmation == 'n')
                            {
                                break;
        ...
        } else if(organizer_choice == 7) // maanage reminders
        {
            ...
            while(1)
            {
                ...
                } else if (option == 2) // edit user
                {
                    ...
                    while(1)
                    {
                        ...
                        printf("              👥 Edit Users                     \n");
                        printf("==================================================\n");
                        printf("📋 In this section, you can view, edit, or manage\n");
                        printf("==================================================\n");
                        reveal_all_users_except_organizers();

                        if(scanf("%d", &user_id) != 1 || valid_user_id(user_id) != 1 && user_id != -1)
                        {
                            printf("\n Please enter a valid user id\n");
                            Sleep(2000);
                            clear_input_buffer();
                    ...
                    /* code */
                } else if (option == 3) // view all user details
                {
                    int user_id;
                    ...
                    while(1)
                    {
                        ...
                        while(1)
                        {
                            ...
                            {
                                buffer_view[i] = malloc(100);
                                const char *value = read_user(user_id, fields[i].key);
                                if (value) 
                                {
                                    sprintf(buffer_view[i], "✨ %-15s: \t%s", fields[i].display_name, value);
                                } else 
                    ...
                    /* code */
                } else if (option == 4) //back to dashboard
                {
                   break;
                }
                   
            }
        } else if(organizer_choice == 8) // notifcation and logs
        {
            while(1)
            {
                int menu_choice;
            ...
            while (1)
            {
                ...
                else if (organizer_choice == 2) // Manage Tickets
                {
                    while(1)
                    {
                        ...
                        system("cls");
                        printf("==================================================\n");
                        printf("              🎟️ Manage Tickets                   \n");
                        printf("==================================================\n");
                        printf("\n📋 In this section, you can manage client support tickets effectively:\n");
                        printf("- 📂 View ticket details, including the issue reported.\n");
                        printf("- ✏️ Update the status of a ticket (e.g., resolved, pending).\n");
                        printf("- 📧 Communicate resolutions or updates to clients.\n");
                        printf("\n💡 Use this tool to ensure smooth and transparent support processes.\n");
                        printf("\n==================================================\n");
                        reveal_all_users_except_organizers();
                        scanf("%d", &user_id);
